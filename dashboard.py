...
def load_sentiment_pipeline():
    try:
        return pipeline("sentiment-analysis", model="models/twitter-roberta-base-sentiment")
    ...
    if st.button("📥 Fetch & Analyze Post") and post_link and ig_user:
        try:
            ...
            try:
                ...
                st.warning("⚠️ Session file not found. Some data may be limited for private accounts.")
            ...
            try:
                ...
                col1, col2, col3 = st.columns(3)
                col1.metric("👍 Predicted Likes", f"{predicted_likes:,}")
                col2.metric("💬 Predicted Comments", f"{predicted_comments:,}")
